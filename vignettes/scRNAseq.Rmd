...
SRA files were downloaded from the Sequence Read Archive and the SRA Toolkit (v. xx) was used to transform them to FASTQ. FASTQ files were downloaded from EMBL-EBI ArrayExpress.

Reads were aligned with TopHat (v. xx) [@trapnell2009tophat] to the appropriate reference genome (GRCh38 for human samples, GRCm38 for mouse). RefSeq mouse gene annotation (GCF_000001635.23_GRCm38.p3) was downloaded from NCBI on Dec. 28, 2014. RefSeq human gene annotation (GCF_000001405.26) was downloaded from NCBI on Feb. 23, 2015.

featureCounts (v. xx) [@liao2013featurecounts] was used to compute gene-level read counts and Cufflinks (v. xx) [@trapnell2010transcript] was used to compute gene-leve FPKM's.
...
In addition to the gene expression levels, the object contains some useful quality control (QC) information, as well as the available annotation of the samples. This information can be accessed through the `colData` accessor.
