...
meta <- read.table("tmp/E-MTAB-2512.sdrf.txt", as.is=TRUE, header=TRUE, sep='\t')
library(SummarizedExperiment)

th2 <- SummarizedExperiment(assay=list(counts=gene_counts, fpkm=gene_fpkm),
                           colData=DataFrame(t(qc)), metadata=list(meta))

save(th2, file="data/th2.rda")
